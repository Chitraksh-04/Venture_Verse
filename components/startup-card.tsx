...
const StartupCard = ({ post }: { post: StartupTypeCard }) => {
  const {
   ...
   console.log("Author image field:", author?.image)
  console.log("Author image URL:", author?.image ? urlFor(author.image).url() : "No image")
      ...
      <CardContent className="p-4 pt-0">
        <div className="mb-4 flex items-start justify-between gap-4">
          ...
          <Link href={`/user/${author?._id}`}>
            <Avatar className="size-12">
              {author?.image ? (
                <AvatarImage
                  src={urlFor(author.image).width(200).height(200).url()} // ✅ fixed
                  alt={author?.name}
                />
              ) : (
                <AvatarFallback>
        ...
        <Link href={`/startup/${_id}`} className="block">
          ...
          <div className="relative h-48 w-full overflow-hidden rounded-lg">
            {image ? (
              <Image
                src={urlFor(image).width(800).height(600).url()} // ✅ fixed
                alt={title as string}
                fill
                className="object-cover transition-transform duration-300 ease-in-out group-hover:scale-105"
              />
            ) : (
              <div className="flex h-48 w-full items-center justify-center rounded-lg bg-gray-800 text-gray-400">
                No Image
              </div>
            )}
          </div>
